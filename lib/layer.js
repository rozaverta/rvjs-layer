...
function Call(name, func, ...evn)
{
	let layer = Reg[name] || {};

	if( typeof layer[func] !== "function" ) {
		return func === 'reload' ? Call(name, 'load', ...evn) : void 0
	}
...
{
	if( data !== null ) {
		let found = LayerFound(name, 'name');
		if( found > -1 ) {
...
{
	let history = Reg[name].history,
		valid = () => ! IsWait && Layer.name === name;
	...
	return {
		...
		{
			if( valid() ) {
				let copy = UpdateHistory(name, data);
				if(copy) {
...
function LayerLoad(name, data, complete)
{
	let found = LayerFound(name, 'name'),
		...
		renewed = ! reload,
		component = Reg[name],
		history = component instanceof ComponentHistory;

	if(history) {
		let h = component.history;
	...
	let c = () => { console.log("complete open", cur, cur.payload);
...
function LayerDestroy(layer, blur, complete)
{
	...
	let index = LayerFound( layer.uid, 'uid' ), // found index
		close = () => {
			try {
...
const Layer =
{
	...
	fire(name, ...args)
	{
		...
		if(Layer.isOpened && privates.indexOf(name) < 0) {
			return Call(Layer.name, name, ...args)
		}
	},
	...
	focus(name)
	{
		let not = NotWait(),
			f = complete => {
				let found = LayerFound(name, 'name'), // found layer
					end = Layers.length - 1;
